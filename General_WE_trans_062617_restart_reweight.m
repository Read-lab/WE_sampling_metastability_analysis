...
temp_prob = VVs(:,1)./sum(VVs(:,1));
for i = 1:num_nodes
    len = length(replicas_forwards(bins==i,end));
    replicas_forwards(bins==i,end) = temp_prob(i)./len;
end
...
unix([ ' > ' temp_save_location '/finished.txt']);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%
    temp= regexp(fileread('sub_BNG_scripts_batch_051217.sh'),'\n','split');
    ...
    fid = fopen(['sub_BNG_scripts_batch_051217.sh'],'w');
    fprintf(fid,'%s \n',temp{:});
    fclose(fid);
    ...
    if rem(length(Binsprevx),bash_clump) ~=0
        ...
        fid = fopen(['sub_BNG_scripts_batch_051217_fin.sh'],'w');
        fprintf(fid,'%s \n',temp{:});
        fclose(fid);
        unix(['qsub sub_BNG_scripts_batch_051217_fin.sh']);
        
    end
    
    unix(['qsub sub_BNG_scripts_batch_051217.sh']);
